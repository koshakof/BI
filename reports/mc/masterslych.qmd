...
account %>% 
  ...
  geom_label_repel(aes(x = as.Date("2023-07-24"), y = 0,
            ...
            label.padding = 0.5, 
            direction = "y")+
  geom_label_repel(aes(x = as.Date("2023-07-31"), y = 0,
                label = "24 Июль. \n Вкл «Макс. кликов"),
            stat = "unique",
            size = 2.5, 
            color = "gray20",
...
DT::datatable(camp_month,
              ...
              caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left; color: gray;',
                                                htmltools::h4('Таблица: Срез по рекламной кампании')),
              options = list(columnDefs = list(list(className = 'dt-right', targets = 4:6),
...
datatable(groups_7,
          ...
          caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left; color: gray;',
                                            htmltools::h4('Таблица: Данные по группам и кампаниям')),
          options = list(columnDefs = list(list(className = 'dt-right', targets = 3:5)),
                             pageLength = 10,
                             dom        = "Btpli",
                             language   = list(url = '//cdn.datatables.net/plug-ins/1.13.6/i18n/ru.json'),
                             buttons    = c('csv', 'excel', 'pdf'),
...
account %>% 
  reframe(across(where(is.numeric), sum), .by = CampaignName) %>% 
  select(CampaignName, Clicks) %>% 
  ggplot(aes(x = reorder(CampaignName, -Clicks), y = Clicks, group = 1)) +
  geom_point() +
  geom_line() +
  coord_polar() +
  theme_minimal() +
...
```{r get data ads, include = F, eval = F}
...
ads_df %>% 
  kbl(format = "html", 
      caption = "Метрики объявлений у которых больше 29 кликов", 
      escape = F) %>% 
  kable_paper() %>% 
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), 
                fixed_thead = TRUE) %>% 
  row_spec(0, 
           ...
           color = "#5A5A5A", 
           font_size = 16) %>% 
  row_spec(1:length(slots$ads_df),
           align = "r",
           font_size = 14) %>%
  column_spec(c(3, 6),
              border_right = T) %>%
...
```{r get data cliclType, include = FALSE, eval = FALSE}
...
```{r temp7, include = FALSE, eval = FALSE}
...
```{r get data segments, include = F, eval = F}
